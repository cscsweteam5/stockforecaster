<?php
   class DB_Functions {
      ...
      //query to view stocks in Stock Info
	  public function viewStocks(){
		  $conn = $this->conn;
		  $result = $conn->query("SELECT `Stock_ID`,`Stock`, `Symbol`, `Exchange`, `Price`, `High`, `Low`,
		                                 `Daily_Change`, `Weekly_Change`, `Monthly_Change`, `Prediction`
		                          FROM `stocks`");
          if($result->num_rows === 0) 
			  print_r('No rows');
          while($row = $result->fetch_assoc()) {
             print "" . $row["Stock_ID"]. " - Stock Name: " . $row["Stock"]. " - Symbol: ". $row["Symbol"].
                    " - Exchange: ". $row["Exchange"]. " - Price: ".$row["Price"]. " - High: " . $row["High"].
					" - Low: " . $row["Low"]. " - Daily: " . $row["Daily_Change"]. " - Weekly: " . $row["Weekly_Change"].
					" - Monthly: " . $row["Monthly_Change"]. " - Prediction: " . $row["Prediction"];
			 print "<br></br>";
		  }
	  }	  	  
   }
